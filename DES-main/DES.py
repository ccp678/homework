...
def generate_key(key):
    ...
    key_left = key[0:28]
    key_right = key[28:]
    #print('left',len(key_left))
    #print('right',len(key_right))
...
def start():
    ...
    if t == '0':
        ...
        with open("result.txt",'w',encoding="uft-8") as f:
            f.write(result_str)
        f.close()
    elif t == '2':
        print("Exit!")
...
def test():
    print("\nEncrypt press 0   Decrypt press 1   Exit press 2:")
    for t in ['0','1','2']:
        if t == '0':
            print("Input the Plaintext Filename: ")
            message = 'des.txt'  #明文文件
            message = read_file(message)  #.replace(" ","")
            print("Input the password you want to set:")
            key = 'message'   #密文
            #print("Plaintext:  "+message)
            #print("Plaintext 01bits:   " + str_bit(message))
            result = all_des_encrypt(message, key)
            ...
            print("Input the Ciphertext Filename: ")
            message = 'text.txt'
            message = read_file(message)
...
def image_en_de(key):
    ...
    with open('des.txt','r') as f:
        with open("result.txt",'w',encoding="utf-8") as f2:
            f2.write('')
        f2.close()
        f3 =  open("result2.txt",'a+',encoding="utf-8")
            #f3.write('')
        ...
        while(True):
            ...
            result_str = bit_str(result)
            write_file(result_str)
            mes_str_bit = str_bit(message)
            f3.write('加密前：'+mes_str_bit+'\n')
            f3.write('加密后：'+result+'\n')
            l = len(mes_str_bit)
            ...
            result_str = bit_str(result) 
            f3.write('解密后：'+result+'\n')
            if mes_str_bit == result:
                f3.write('True'+'\n')
            else:
                f3.write('False\n')
                #print(i,'false')
                result_str = bit_str( mes_str_bit)
            #print("Plaintext:   " + result_str)
